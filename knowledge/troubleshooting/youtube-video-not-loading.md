...
tags: youtube, googlevideo, CDN, video, troubleshooting, badsum, aggressive, desktop
created: 2026-03-29
updated: 2026-03-29
...
YouTube использует двухфазную модель соединений:

1. **Фаза 1 — страница:** TLS-соединения с `youtube.com`, `youtubei.googleapis.com`. Стандартный DPI-обход (split2, disorder2) обычно достаточно.

2. **Фаза 2 — видеопоток:** Отдельные TLS-соединения с CDN-серверами `googlevideo.com` (домены вида `rrXX---sn-*.googlevideo.com`). DPI инспектирует эти соединения **агрессивнее**, и слабая стратегия не проходит.

Таким образом, стратегия, которая пробивает youtube.com, может не пробивать googlevideo.com CDN. Нужна более агрессивная стратегия для видеопотока.
...
QUIC (UDP 443) часто блокируется или дропается для YouTube. Если QUIC не обрабатывается zapret2, браузер может пытаться использовать его вместо TCP и зависать.
...
**Что делает:**
- `multidisorder` — разбивает ClientHello в 7 позициях (SNI extension, hostname, mid-SLD × 3, конец hostname), отправляет сегменты в обратном порядке
- `fake` с `badsum` — отправляет 4 поддельных ClientHello с испорченной TCP-контрольной суммой. DPI их видит, сервер — отбрасывает
...
zapret2 включает предсобранный профиль `youtube` с 14 вариантами стратегий в circular-ротации. Если одна стратегия не работает — автоматически переключается на следующую.
