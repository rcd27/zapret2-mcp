...
1. **`ip6_ttl=N`** - установить Hop Limit в IPv6 заголовке (аналог TTL)
    - Пример: `ip6_ttl=3`

2. **`ip6_autottl=delta,min-max`** - автоматическое определение Hop Limit для IPv6
    - Формат: `delta,min-max`

### 🔹 **IPv6 Extension Headers (расширенные заголовки)**

1. **`ip6_hopbyhop[=hex]`** - добавить Hop-by-Hop заголовок
    - Размер данных: 6+N×8 байт
    - По умолчанию: `\x00\x00\x00\x00\x00\x00`

2. **`ip6_hopbyhop2[=hex]`** - добавить второй Hop-by-Hop заголовок
    - Размер данных: 6+N×8 байт

3. **`ip6_destopt[=hex]`** - добавить Destination Options заголовок (unfragmentable part)
    - Размер данных: 6+N×8 байт

4. **`ip6_destopt2[=hex]`** - добавить второй Destination Options заголовок (fragmentable part)
    - Размер данных: 6+N×8 байт

5. **`ip6_routing[=hex]`** - добавить Routing заголовок
    - Размер данных: 6+N×8 байт

6. **`ip6_ah[=hex]`** - добавить Authentication Header (усеченный)
   - Размер данных: 6+N×4 байт
   - По умолчанию: `\x00\x00` + 4 случайных байта

### 🔹 **TCP модификации**

1. **`tcp_seq=N`** - добавить N к TCP sequence number
   - Пример: `tcp_seq=10000`
   - Используется для "badseq" обманки

2. **`tcp_ack=N`** - добавить N к TCP acknowledgment number
   - Пример: `tcp_ack=-66000`
   - Используется для "badack" обманки (обычно отрицательное значение)

3. **`tcp_ts=N`** - добавить N к TCP timestamp value
   - Модифицирует TCP опцию timestamp

4. **`tcp_md5[=hex]`** - добавить TCP MD5 опцию (RFC 2385)
   - Размер: 16 байт
   - По умолчанию: случайные данные
   - Пример: `tcp_md5` или `tcp_md5=0x00112233445566778899aabbccddeeff`

5. **`tcp_flags_set=<список>`** - установить TCP флаги
   - Список через запятую: `FIN,SYN,RST,PSH,ACK,URG,ECE,CWR`
   - Пример: `tcp_flags_set=PSH,URG`

6. **`tcp_flags_unset=<список>`** - снять TCP флаги
   - Пример: `tcp_flags_unset=ACK` (для "datanoack" обманки)

7. **`tcp_ts_up`** - переместить TCP timestamp опцию в начало
   - Без параметров
   - Workaround для Linux: позволяет отбрасывать пакеты с badack без badseq

### 🔹 **Дополнительные опции**

1. **`badsum`** - сделать контрольную сумму L4 невалидной
   - Без параметров
   - Относится к reconstruct options

2. **`fool=функция`** - вызвать пользовательскую функцию обманки
...
Все эти обманки применяются функцией `apply_fooling()` из библиотеки `zapret-lib.lua` и используются для обхода DPI
систем путем создания пакетов, которые проходят через DPI, но отбрасываются целевым сервером или наоборот.
